"""FastAPI application - MVP with in-memory storage."""
...
# === In-Memory Storage (replace with PostgreSQL in Phase 2) ===
...
app = FastAPI(title="E-Commerce AI Studio", version="0.1.0", lifespan=lifespan)
...
async def generate_image(req: GenerateReq, user: dict = Depends(get_current_user)):
    ...
    # Simulate async generation (replace with real pipeline when LLM API key is set)
    async def simulate():
        ...
        tasks_db[task_id] = {
            **tasks_db[task_id],
            "status": "completed",
            "progress": 1.0,
            "result_urls": ["https://picsum.photos/1024/1024"],
        }
...
async def list_models():
    return [
        ...
        {"name": "mock-image", "provider_type": "image", "provider": "mock", "cost_per_image": 0.0, "is_enabled": True},
